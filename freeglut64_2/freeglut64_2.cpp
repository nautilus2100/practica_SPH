// SPH64.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
//
// glut2.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
//
...
const static float POLY6 = 315.f / (65.f * 3.1416 * pow(H, 9.f));
const static float SPIKY_GRAD = -45.f / (3.1416 * pow(H, 6.f));
const static float VISC_LAP = 45.f / (3.1416 * pow(H, 6.f));
...
const static int BLOCK_PARTICLES = 50;
...
void InitGL(void)
{
	glClearColor(0.9f, 0.9f, 0.9f, 1); // color fondo ventana
...
int main(int argc, char** argv)
{
	...
	glutInit(&argc, argv);
	glutCreateWindow("CELLA Muller SPH");
